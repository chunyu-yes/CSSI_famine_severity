...
lm(china_pop2[1:8,2]~china_pop2[1:8,1])
#slope 5623 intercept -10817971
lm(china_pop2[14:21,2]~china_pop2[14:21,1])
#slope -165 intercept 562901
...
# We did not supply the raw population data on prefecture level to calculate the CSSI
#######################################################################
#                       Input data
#######################################################################
...
# Extract CSSI data from last part
...
china_map_prefecture$NAME_2[which(is.na(china_map_prefecture$X_cssi_Pre)==TRUE)]
china_map_prefecture$NAME_1[which(is.na(china_map_prefecture$X_cssi_Pre)==TRUE)]
# 7 prefecture-level CSSI for Taiwan are missing.
...
map3 <- ggplot() +
  geom_sf(data = china_map_prefecture,size=0.2,aes(fill =cut(X_cssi_Pre,c(-Inf,10,20,30,40,50,60,70,Inf)))) + 
...
map4 <- ggplot() +
  geom_sf(data = china_map_prefecture,size=0.2,aes(fill =cut(X_cssi_Pre.post,c(-Inf,10,20,30,40,50,60,70,Inf)))) + 
  geom_sf(data = china_map_province,size=0.5,color='#292421',fill=NA)+
  coord_sf(crs = "+proj=aeqd +lat_0=37 +lon_0=104")+
  scale_fill_manual(name='CSSI',values=cbPalette,guide=guide_legend(reverse=T),na.value ="white")+
  theme_classic()+theme(axis.line=element_blank(),axis.text.x=element_blank(),axis.text.y=element_blank(),axis.ticks=element_blank(),axis.title.x=element_blank(),axis.title.y=element_blank(),panel.border = element_blank())
...
china_map_prefecture2$X_cssi_Pre[which(china_map_prefecture$Province=='Beijing'|china_map_prefecture$Province=='Tianjin'|china_map_prefecture$Province=='Shanghai'|china_map_prefecture$Province=='Xinjiang'|china_map_prefecture$Province=='Tibet')]<-NA
china_map_prefecture2$X_cssi_Pre.post[which(china_map_prefecture$Province=='Beijing'|china_map_prefecture$Province=='Tianjin'|china_map_prefecture$Province=='Shanghai'|china_map_prefecture$Province=='Xinjiang'|china_map_prefecture$Province=='Tibet')]<-NA
...
map6 <- ggplot() +
  geom_sf(data = china_map_prefecture2,size=0.2,aes(fill =cut(X_cssi_Pre-X_cssi_Pre.post,c(-Inf,-20,-15,-10,-5,5,10,15,20,Inf))))+ 
